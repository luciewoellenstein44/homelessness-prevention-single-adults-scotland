...
This rmarkdown script produces Odds Ratio plots. Here it was done for the Glasgow sample of PREVENT1 data set. The odds ratios (ORs) came from the Logistic Regression (LR) and Elastic Net (ENet) models produced by  '/research/analysis/prediction_models.rmd'. The OR graph only includes ORs that had significant p-values in the LR model and were included in both the LR and ENet models. 
...
```{r}
#Import
or_data <- read_excel("data/GLA_HOO_OR.xlsm", # this excel file has LR and ENet coefficients and ORs. Data folder is inside visualise folder
...
```{r, eval=FALSE}
ggsave("or_plot_GLAHOO.png", combined, width = 15, height = 22, 
       bg = "white")
```
